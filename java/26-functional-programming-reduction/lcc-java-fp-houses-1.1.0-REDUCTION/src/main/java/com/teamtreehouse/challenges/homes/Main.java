...
public class Main {
  ...
  public static OptionalInt getLowestHomeValueIndexDeclaratively(List<HousingRecord> records) {
    // DONE: Create a stream off the records
    // DONE: Map the stream to an IntStream on `HousingRecord.getCurrentHomeValueIndex` to expose new methods
    // DONE: Return the minimum value from the stream.  It should be an optional because records could be empty.
  ...
  public static Optional<HousingRecord> getHighestValueIndexHousingRecordDeclaratively(List<HousingRecord> records) {
    // DONE: Create a stream off of the records
    // DONE: Find and return the maximum using a Comparator that compares the current home value index
    return records.stream()
