...
function printNameVar(f, l) {
    ...
    var greeting : string = 'Hello there, ';  // OK to redefine var, but if we declare as let throws an error: not allowed to redefine let variables
    console.log(greeting + f + ' ' + l);
...
function printNameLet(f, l) {
    let greeting : string = 'Hi there, '; // Change to let works the same.
    console.log(greeting + f + ' ' + l);
